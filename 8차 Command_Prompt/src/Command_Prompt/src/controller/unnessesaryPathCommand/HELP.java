...
public class HELP implements ExcutionCommand
{
    ...
    public HELP(InputDTO inputDTO, ExceptionHandler exceptionHandler)
    {
    ...
    public void excuteCommand()
    {
        ...
        if(isHelp)
        {
            String commandList = "특정 명령어에 대한 자세한 내용이 필요하면 HELP 명령어 이름을 입력하십시오.\n" +
                ...
                PrinterMessage.getPrinterMessage().printMessage(commandList);
        }
        else
        {
            inputValue = inputDTO.getTotalInput().replace(" ", "");
            inputValue = inputValue.substring(4);
            PrinterMessage.getPrinterMessage().printExceptionMessage(Constant.HELP_CONTAINS_ANOTHER, inputValue);
        }
    }
}
